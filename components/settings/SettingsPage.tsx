...
function TG({l,d,v,c}:{l:string;d:string;v:boolean;c:(v:boolean)=>void}){
  return(
    <div className="flex items-center justify-between border-b border-border py-3 last:border-b-0">
      <div><p className="text-[13px] font-semibold text-text">{l}</p><p className="text-[11px] text-text-3">{d}</p></div>
      <div onClick={()=>c(!v)} className={`relative cursor-pointer rounded-full transition-colors ${v?"bg-orange":"bg-border"}`} style={{width:44,height:24,flexShrink:0}}>
        <div className="absolute rounded-full bg-white shadow transition-all" style={{width:18,height:18,top:3,left:v?23:3}}/>
      </div>
    </div>
  );
}
